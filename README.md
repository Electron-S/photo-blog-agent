...
**수렴 판정 근거** (`test/lint-draft.test.js`의 "새 표면형 계량" 테스트가 단언):
정당한 표현에서 error 차단 **0건**(차단은 발행 불가라 가장 비쌉니다), 위반에서
error 적중 **하한 12건**, 그리고 **error도 warn도 놓치는 것이 0건**입니다.
수치가 나빠지면 그 테스트가 깨져 설계를 다시 보게 됩니다.
...
| `get-blogger-token.js` | 토큰 저장 완료 | 인자·포트·토큰 교환 실패 | 토큰은 발급됐으나 `.env` 저장 실패 (토큰을 stderr에 출력 — 재발급 불필요) | — | — | — | — | — | — |
