...
// sharp가 디코드/인코딩에 실패했을 때의 최소 폴백. **오직 sharp 실패만** 여기로 온다 —
// 치수 계약 위반이나 디스크 오류가 섞이면 원인이 "sharp 인코딩 실패"로 잘못 표기된다.
...
async function compressImage(inputPath, outputPath, format = 'webp', maxSizeKB = DEFAULT_MAX_SIZE_KB) {
  ...
  // sharp 디코드/리사이즈만 이 try로 감싼다. 예전에는 아래 치수 검증과 품질
  // 루프까지 같은 try 안에 있어서, **자기가 세운 가드를 자기 catch가 삼켰다** —
  // 치수 검증 throw든 writeFile의 ENOSPC든 전부 "sharp 인코딩 실패"로
  // 재라벨링되어 워터마크 없는 원본이 fallback으로 올라갔다.
  let resized;
  let resizedMeta;
  try {
  ...
  // 이건 폴백 대상이 아니라 계약 위반이다. 예전에는 `|| 1600` / `|| 1200` 으로
  // 폴백했는데, 그러면 워터마크 SVG를 실제와 다른 캔버스 크기로 그려 엉뚱한
  // 위치에 합성하고 그 거짓 치수가 <img width height>까지 흘러가 CLS를 유발한다.
  // 모르면 모른다고 실패한다 — catch 밖이므로 호출자까지 그대로 올라간다.
...
async function uploadBlogImages(inputPaths, options = {}) {
  ...
  const images = await mapWithConcurrency(inputPaths, concurrency, async (inputPath, i) => {
    ...
    try {
      ...
      console.error(`[uploadBlogImages] 이미지 ${i + 1} (${inputPath}) 실패: ${err.message}`);
