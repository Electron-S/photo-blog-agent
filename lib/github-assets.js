...
async function compressImage(inputPath, outputPath, format = 'webp') {
  ...
  try {
    ...
    console.warn(`[compressImage] sharp ${format} failed, copying original WITHOUT resize/compression: ${inputPath} — ${err.message}`);
...
async function uploadBlogImages(inputPaths, options = {}) {
  ...
  for (let i = 0; i < inputPaths.length; i += 1) {
    ...
    try {
      ...
      const webpUrl = await uploadAsset(webpPath, `posts/${postDir}/${webpName}`);
