...
async function uploadAsset(localPath, remotePath) {
  ...
  try {
    ...
    if (err.response?.status !== 404) {
      throw new Error(`GitHub 파일 확인 실패 (${remotePath}, branch ${branch}): ${err.response?.status} ${errFull(err)}`);
    }
  }
  ...
  try {
    ...
    const detail = err.response?.data?.message || errFull(err);
...
async function uploadBlogImages(inputPaths, options = {}) {
  ...
  if (typeof options.slug !== 'string' || !options.slug.trim()) {
    throw new Error(
      'uploadBlogImages: options.slug이 필요합니다. 없으면 같은 날짜의 모든 글이 '
      + `같은 폴더(${postDirName(date, 'post')})를 공유해 이미 발행된 글의 이미지를 덮어씁니다.`,
    );
  }
  if (slugCollapsed(options.slug)) {
    throw new Error(
      `uploadBlogImages: slug "${options.slug}"이 ASCII 경로로 바꾸는 과정에서 전부 사라집니다 `
      + "(→ 'post'). 영문/숫자/하이픈으로 된 slug를 쓰세요 — 그러지 않으면 서로 다른 글이 "
      + '같은 폴더를 덮어씁니다.',
    );
  }
  ...
  const images = await mapWithConcurrency(inputPaths, concurrency, async (inputPath, i) => {
    ...
    try {
      ...
      const isContractError = err && err.kind === 'contract';
      const isCompressionError = !isContractError && errFull(err).startsWith('compressImage');
      return {
        index: i + 1,
        originalPath: inputPath,
        webpPath: null,
        ...
        watermarkApplied: false,
        orientationApplied: false,
        compressionError: (isCompressionError || isContractError) ? errFull(err) : null,
