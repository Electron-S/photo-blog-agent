...
async function uploadBlogImages(inputPaths, options = {}) {
  ...
  // 기본 경로는 postDir을 **포함**한다 — 그 세그먼트가 글마다 로컬 산출물을
  // 분리하는 유일한 수단이다. `--work-dir`를 주면 그 경로를 그대로 쓰므로
  // 분리가 사라진다: 서로 다른 글을 같은 --work-dir로 돌리면 photo-NN.webp가
  // **조용히 덮어써지고**, 먼저 만든 글의 upload-<slug>.json에 적힌 webpPath가
  // 나중 글의 사진을 가리킨다 (실측 재현). 네이버 발행은 그 경로의 파일을
  // 그대로 에디터에 올리므로 다른 글의 사진이 올라간다.
  //
  // 차단하지는 않는다 (명시적 override다 — lib/naver-content.js의 --image-dir
  // 불일치와 같은 취급). 대신 분리가 사라졌다는 사실을 표면화한다.
  const workDir = options.workDir || path.join(process.cwd(), 'tmp', 'assets', postDir);
  if (options.workDir && path.basename(path.resolve(options.workDir)) !== postDir) {
    console.warn(`[uploadBlogImages] --work-dir가 글 폴더(${postDir})가 아닙니다: `
      + `${options.workDir}\n`
      + '  + 글마다 photo-NN.webp 이름이 같아서, 다른 글을 같은 경로로 돌리면 덮어씁니다.\n'
      + '  + 결과 JSON의 webpPath가 나중 실행에 의해 다른 글의 사진을 가리킬 수 있습니다.\n'
      + `  + 분리를 유지하려면 --work-dir를 생략하거나 .../${postDir} 로 지정하세요.`);
  }
