...
//
// **이 파일에는 살아 있는 인증 쿠키(NID_AUT/NID_SES)가 들어 있다.**
// `context.storageState()`는 쿠키 저장소를 항상 포함한다. gitignore 대상이지만
// 기본 권한(0644)으로 쓰면 프로젝트 디렉터리를 tar로 묶거나 .gitignore를 보지 않는
// 도구가 **동작하는 네이버 세션**을 그대로 실어 나른다. 0600으로 쓴다.
  ...
  try {
    ...
    fs.writeFileSync(SESSION_SNAPSHOT, `${JSON.stringify({
      _comment: '감사/디버그용 스냅샷입니다. 로그인 판정에는 쓰이지 않습니다 (.naver-profile/이 실제 세션). '
        + '**살아 있는 인증 쿠키가 들어 있으니 공유·업로드하지 마세요.**',
