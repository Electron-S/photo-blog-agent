...
// 예외의 첫 줄을 안전하게 뽑는다. Playwright/Node는 Error가 아닌 값을 던지거나
// reject할 수 있고(문자열 throw, undefined reject), err.message를 무가드로 만지면
// 거기서 TypeError가 나 **원래 예외의 exitCode가 통째로 사라진다**. 그러면
// naver-doctor의 `fail(err.exitCode || SESSION)`이 전부 11로 강등해, 네트워크
// 문제를 겪는 사용자에게 재로그인을 반복시킨다. lib/naver-dom.js가 같은 이유로
// 같은 가드를 두고 있다.
...
async function openContext({ headless = false, dir = profileDir() } = {}) {
  ...
  try {
    ...
    if (/SingletonLock|ProcessSingleton|already (in use|running)/i.test(err.message)) {
      throw new NaverError(
        NAVER_EXIT.GENERAL,
...
async function saveSessionSnapshot(context, { blogId } = {}) {
  try {
    ...
    console.warn(`[naver] 세션 스냅샷 저장 실패(무시): ${err.message}`);
    return null;
  }
}
