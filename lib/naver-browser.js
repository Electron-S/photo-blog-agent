...
function displayAvailable() {
  ...
  if (fs.existsSync('/tmp/.X11-unix/X0')) return { ok: true, via: '/tmp/.X11-unix/X0' };
...
function assertDisplayAvailable() {
  ...
  if (!d.ok) {
    throw new NaverError(
      ...
      'headed 브라우저를 띄울 수 없습니다 (DISPLAY / WAYLAND_DISPLAY / X 소켓 모두 없음).\n'
...
async function hasLoginCookies(context) {
  ...
  try {
    ...
    throw new NaverError(
      NAVER_EXIT.GENERAL,
      `쿠키를 조회하지 못해 로그인 여부를 판정할 수 없습니다: ${err.message}\n`
