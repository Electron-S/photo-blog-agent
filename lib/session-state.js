...
// `explicit`: 호출자가 --dir를 **명시했는가**. 명시했는데 없으면 경로 오타이므로
// 실패로 알리고, 기본 tmp/가 아직 없는 것(갓 클론한 저장소)은 정상이다.
function listStates(dir = 'tmp', { explicit = false } = {}) {
  let files;
  try {
    ...
    if (errCode(err) === 'ENOENT') {
      if (!explicit) return [];   // 기본 tmp/가 아직 없는 것은 정상이다
      throw new SessionStateError(
        `상태 디렉터리가 없습니다: ${dir}\n`
        + '  + --dir 경로를 확인하세요. "진행 중 세션 없음"과 구별하기 위해 실패로 알립니다.',
      );
    }
    throw err;
  }
