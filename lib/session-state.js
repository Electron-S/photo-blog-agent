...
// publish-post.js의 슬러그 검증(^[a-zA-Z0-9-]+$)과 정렬한 소문자 버전.
...
function normalizeState(raw, { slug, strict = false }) {
  ...
  if (!Array.isArray(raw.steps_completed)) {
    warnings.push('steps_completed가 배열이 아니어서 빈 배열로 간주합니다.');
  }
...
//
// 디스크에 닿는 유일한 함수이므로 여기서 불변식을 강제한다. 예전에는 임의 객체를
...
function writeStateAtomic(slug, dir, state) {
  const { state: validated } = normalizeState(state, { slug, strict: true });
  const p = statePath(slug, dir);
