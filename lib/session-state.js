...
const { SLUG_RE, isValidCalendarDate } = require('./slug');
...
// `schemas/session-state.schema.json`이 `^\d{4}-\d{2}-\d{2}$`를 선언하는데
// 무검증 대입이었다 — `--primary-date "2026/05/10"`·`"not-a-date"`가 exit 0으로
// 디스크에 남았고, 재개하는 다른 세션·모델이 그 값을 **방문 날짜**로 읽는다.
// slug은 SLUG_RE로 정확히 강제되는데 패턴 제약이 있는 필드 중 이것만 빠져 있었다.
function assertPrimaryDate(value) {
  if (value === null || value === undefined) return null;
  if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    throw new SessionStateError(
      `primary_date는 YYYY-MM-DD 형식이어야 합니다 (받음: ${JSON.stringify(value)}). `
      + '이 값이 블로그 글의 방문 날짜이자 폴더 경로의 날짜가 됩니다.',
    );
  }
  // 달력 검사는 lib/slug.js의 정본을 쓴다. 예전에는 여기서 손으로 재구현했고,
  // 같은 검사가 scripts/upload-images.js에도 또 복제돼 있었다 — 그 사이에
  // checkDatePlausible만 이 검사가 없어서 EXIF 경로가 2026-02-30을 통과시켰다.
  const [y, mo, d] = value.split('-').map(Number);
  if (!isValidCalendarDate(y, mo, d)) {
    throw new SessionStateError(`primary_date "${value}"는 유효한 달력 날짜가 아닙니다.`);
  }
  return value;
}
...
function defaultState(slug, fields = {}) {
  return {
    ...
    primary_date: assertPrimaryDate(fields.primary_date ?? null),
    steps_completed: [],
...
function normalizeState(raw, { slug, strict = false }) {
  const warnings = [];
  // 경고에는 두 종류가 있고 **조치가 다르다**:
  //   양성  — 다음 저장 때 채워지는 것 (schema_version 누락, steps_remaining 재계산)
  //   손상  — 데이터가 사라졌거나 불변식이 깨진 것 (배열이 아님, 구멍, post_id 불일치)
  // 예전에는 둘을 구별하지 않아서, `read --field steps_completed`가 손상 파일에서
  // **빈 문자열 + exit 0**을 돌려줬다 — 갓 init한 새 세션과 구별할 수 없다.
  // `.claude/commands/blog.md`의 재개 분기가 그 값을 쓰므로 완료된 단계가
  // 소멸하고 두 번째 초안이 만들어진다.
  let degraded = false;
  const degrade = (msg) => { degraded = true; warnings.push(msg); };
  ...
  const state = { ...raw };
  // 디스크에서 읽은 값도 검증한다 — 손으로 편집된 파일이 있을 수 있다.
  if (state.primary_date !== undefined) {
    try {
      assertPrimaryDate(state.primary_date);
    } catch (err) {
      if (strict) throw err;
      degrade(errFull(err));
    }
  ...
  if (state.schema_version === undefined) {
    ...
    degrade(`${msg} ${SCHEMA_VERSION}로 간주합니다.`);
    state.schema_version = SCHEMA_VERSION;
  }
  ...
  if (unknown.length) {
    ...
    degrade(`${msg} — unknown_steps로 격리합니다.`);
...
`
  ...
  return { state, warnings, degraded };
}
...
// 디렉터리가 없으면 빈 배열이다 — "세션이 없다"는 **참**이다.
//
// 한때 `--dir`를 명시했는데 없으면 exit 9를 냈다. 그런데 유일한 호출자인
// `.claude/commands/blog.md`의 진입 프로브가 **항상** 절대경로 `--dir`를 넘기고,
// `tmp/`는 .gitignore 대상이라 **갓 클론한 저장소에는 없다.** 그래서 처음 `/blog`를
// 켜면 존재하지 않는 세션에 대해 "이어서 진행할까요?"를 묻게 됐다 — 면제하려던
// 케이스가 정확히 깨진 케이스였다.
// 경로 오타는 CLI가 stderr 주석으로 알리고(아래 scripts/session-state.js),
// 판정 자체는 "세션 없음"으로 둔다.
function listStates(dir = 'tmp') {
  ...
  try {
    ...
    // **"경로 오타"와 "세션 없음"을 구별한다.** /blog 진입 프로브가 절대경로 --dir로
    // 이 출력만 보고 "이어서/처음부터"를 묻는다. 경로가 틀리면 진행 중인 작업이
    // "없음"으로 읽혀 처음부터 재실행된다.
