...
function readAttributes(html, i) {
  ...
  return { attrs, selfClosing, next: i, malformed };
}
...
function parseHtml(html) {
  ...
  while (i < src.length) {
    ...
    const { attrs, selfClosing, next, malformed } = readAttributes(src, i + om[0].length);
    if (malformed !== null) {
...
function followingSiblings(node) {
  ...
  while (cur && cur.parent) {
    ...
    for (let i = sibs.indexOf(cur) + 1; i < sibs.length; i += 1) out.push(sibs[i]);
