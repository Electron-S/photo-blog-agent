...
function inlineTextOf(node, links) {
  ...
  const rec = (n, isRoot, inAnchor = false) => {
    ...
    if (n.tag === 'a') {
      ...
      // href가 없거나(`<a>텍스트</a>`) 최종화 결과가 비면(`href="   "`,
      // `href="&#8203;"`) **링크가 아니다.** 위 주석의 "조용히 버리지 않는다"는
      // 평문화된 **URL**을 보고하라는 뜻인데 여기엔 보고할 URL이 없고, 라벨
      // 텍스트는 본문에 그대로 남는다 — 잃는 것이 없다. links[]에 href:null을
      // 넣어 보고하는 것도 검토했으나, 본문에 빈 괄호를 남기지 않는다는 기존
      // 계약(test/naver-content.test.js)과 충돌하고 실익이 없어 두지 않는다.
      // 초안의 깨진 <a>는 초안 품질 문제라 lint-draft의 관할이다.
      const hrefText = href ? finalizeSource(href) : '';
...
// 디코드 결과가 **경로 조각 하나**인지 검증한다.
//
// 규약(`{date}-{hash12}` / `photo-NN.webp`)에 정확히 일치할 것을 요구하지는 않는다 —
// 그건 과하다. 실제 위험은 규약을 벗어난 이름이 아니라 **디코드가 만들어내는 구분자**다:
// `%2e%2e%2f`가 `../`가 되어 해석된 경로가 assetsRoot 밖으로 나간다.
// 파일이 없으면 어차피 exit 16으로 걸리므로, 여기서는 그 탈출만 막으면 된다.
...
// percent-escape를 맨눈으로 디코드하면 두 가지가 조용히 깨진다:
//   `%zz` → decodeURIComponent가 raw URIError를 던져 exit 1 "URI malformed"만 남고
//           어느 사진인지·무엇을 하라는지가 사라진다 (이 모듈의 다른 실패는 전부
//           src와 다음 조치를 담는다)
//   `%2e%2e%2f` → `../`가 되어 해석된 경로가 assetsRoot **밖으로 나간다**
// 디코드를 감싸고, 결과를 위 규약으로 검증해서 둘 다 닫는다.
function decodeSegment(seg, src) {
  let out;
  try {
    ...
    throw new NaverError(
      NAVER_EXIT.IMAGE,
      `이미지 src의 경로에 잘못된 percent-escape가 있습니다: "${seg}"\n`
      + `  + src: ${src}\n`
      + '  + 초안 HTML의 이 src를 확인하세요 (업로드 결과 JSON의 webpUrl을 그대로 옮겨야 합니다).',
    );
  }
  return out;
}
...
function localPathFor(src, { assetsRoot, imageDir, warn = () => {} }) {
  const clean = String(src || '').split(/[?#]/)[0];
  const m = /\/posts\/([^/]+)\/([^/]+)$/.exec(clean);
  const srcDir = m ? decodeSegment(m[1], src) : null;
  const srcFile = m ? decodeSegment(m[2], src) : null;

  if (imageDir) {
    // **`{date}-{hash12}`를 버리면 안 된다.** 그 세그먼트가 같은 날 올린 여러 글의
    // `photo-01.webp`를 구별하는 **유일한** 수단이고(lib/asset-paths.js의 해시가
    // 존재하는 이유가 그것이다), 예전에는 basename만 이어붙여 다른 글의 사진을
    // 10/10 해석하고 exit 0으로 끝났다. dry-run 출력도 basename만 찍어서
    // 맞는 글이든 틀린 글이든 **같은 줄**이 나왔다 — 눈으로도 감지할 수 없었다.
    //
    // 차단하지는 않는다 (`--image-dir`는 "압축본 디렉터리를 직접 지정"하는 명시적
    // 탈출구고, 손으로 모은 디렉터리를 가리키는 것이 정당한 용도다). `--date`가
    // 타당 범위 밖 날짜를 경고하며 허용하는 것과 같은 취급으로, **불일치를 보이게**
    // 만든다. 호출자는 전체 경로도 함께 출력한다.
    const dirName = path.basename(path.resolve(imageDir));
    if (srcDir && srcDir !== dirName) {
      warn(`Warning: --image-dir의 디렉터리명이 src의 글 폴더와 다릅니다.\n`
        + `  + src의 글 폴더: ${srcDir}\n`
        + `  + --image-dir:   ${dirName}\n`
        + `  + 같은 날 올린 다른 글의 사진을 올리게 될 수 있습니다 (photo-NN.webp는 글마다 같은 이름입니다).`);
    }
    return path.join(imageDir, basenameOf(clean));
  }

  if (m) {
    // 규약에 맞지 않는 세그먼트는 해석하지 않는다 — `../`로 assetsRoot를 벗어나거나
    // 엉뚱한 파일을 집는 대신 exit 16의 "찾지 못함" 목록에 올려 src를 보여준다.
    if (!isSafePathSegment(srcDir) || !isSafePathSegment(srcFile)) return null;
    return path.join(assetsRoot, srcDir, srcFile);
  }
 ...
 */
function resolveImagePaths(blocks, {
  assetsRoot = path.join('tmp', 'assets'),
  imageDir = null,
  warn = (msg) => console.error(msg),
} = {}) {
  const missing = [];
  const warned = new Set();
  // 같은 경고가 사진마다 반복되면 다른 출력을 밀어낸다 — 한 번만 낸다.
  const warnOnce = (msg) => { if (!warned.has(msg)) { warned.add(msg); warn(msg); } };
  for (const b of blocks) {
    if (b.type !== 'image') continue;
    const p = localPathFor(b.src, { assetsRoot, imageDir, warn: warnOnce });
