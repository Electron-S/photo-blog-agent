...
async function verifyImageUrls(html, { concurrency = 4, retries = VERIFY_RETRIES } = {}) {
  ...
  const checked = await mapWithConcurrency(urls, concurrency, async (url) => {
    let last = null;
    // **실제 시도 횟수를 센다.** 설정값(`retries`)을 그대로 보고하면 404처럼 한 번만
    // 요청하고 포기한 경우에도 "3회 시도"로 찍혀, 나중에 이 값을 출력하는 쪽이
    // 틀린 진단을 낸다 (재시도했는데도 404인 것과 재시도 대상이 아닌 것은 다르다).
    let attempts = 0;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      last = await headOnce(url);
      attempts = attempt;
      if (last.ok) return null;
      if (!isRetriableStatus(last.status) || attempt === maxAttempts) break;
      // 재시도 중 침묵하지 않는다. 응답하지 않는 호스트면 URL당 axios 타임아웃 10초 ×
      // 재시도 + 대기라 수십 초가 아무 출력 없이 흐르고, 사용자는 멈춘 줄 알고
      // Ctrl+C를 누른다. lib/github-assets.js의 verifyAssetUrl과 같은 형식으로 찍는다.
      const why = last.status ? `HTTP ${last.status}` : (last.reason || 'ERR');
      console.warn(`[verifyImageUrls] ${url} 검증 실패 (${attempt}/${maxAttempts}): `
        + `${why}, ${VERIFY_RETRY_DELAY_MS}ms 후 재시도`);
      await new Promise((r) => setTimeout(r, VERIFY_RETRY_DELAY_MS));
    }
    return { url, status: last.status, attempts, reason: last.reason || null };
  });
