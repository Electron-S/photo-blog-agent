...
// 업로드된 이미지가 실제로 네이버 **블로그 저장소**에 올라갔는지 판정하는 호스트.
// blob:/data: URL은 아직 클라이언트 메모리에만 있다는 뜻이라 성공이 아니다.
//
// 반드시 **호스트에 앵커한다.** 예전에는 `/\.pstatic\.net/i`가 섞여 있어
// (1) 앞의 두 패턴을 무의미하게 만들고 (2) 문자열 어디에나 매칭됐다.
// SmartEditor가 업로드 진행 중 넣는 스피너/플레이스홀더는 ssl.pstatic.net ·
// static.pstatic.net에서 오므로, 그것들이 "업로드 완료"로 판정돼
// "실제로 서버에 올라갔다의 유일한 증거"가 거짓 초록이 됐다.
// `//host/path`(프로토콜 상대)도 받는다. getAttribute('src')는 속성 **원문**을
// 돌려주므로, 에디터가 프로토콜 상대 URL을 쓰면 정규화된 절대 URL이 오지 않는다.
// 그 경우 "실제로 올라갔다의 유일한 증거"가 거짓 빨강이 된다.
const NAVER_IMAGE_HOSTS = [
  /^(?:https?:)?\/\/[^/?#]*\bpostfiles\.pstatic\.net(?:[:/?#]|$)/i,
  /^(?:https?:)?\/\/[^/?#]*\bblogfiles\.pstatic\.net(?:[:/?#]|$)/i,
];
