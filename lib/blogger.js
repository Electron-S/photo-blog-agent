...
async function bloggerRequest(method, url, data = undefined) {
  ...
  const oauthDetail = (err) => {
    ...
    return [data?.error_description, code].filter(Boolean).join(' / ') || err.message;
  };
  ...
  const wrapTokenError = (err, context) => {
    ...
    const hint = FIX_HINT[code] || 'BLOGGER_REFRESH_TOKEN / CLIENT_ID / CLIENT_SECRET을 확인하세요.';
    const wrapped = new Error(
      `OAuth 토큰 재발급 실패${context}: ${oauthDetail(err)}\n  + ${hint}`,
    );
