  ...
  do {
    ...
    for (let i = 0; i < candidates.length; i += 1) {
      ...
      try {
        ...
        if (SELECTOR_SYNTAX_RE.test(err.message)) {
        ...
        probeErrors.push(`${candidates[i]}: ${err.message.split('\n')[0]}`);
        count = 0;
      }
...
async function dismissStartupModals(page, frame) {
  for (const modal of SELECTORS.startupModals) {
    for (const scope of [page, frame]) {
      ...
      try { stillVisible = await container.isVisible(); } catch { stillVisible = false; }
      if (stillVisible) {
