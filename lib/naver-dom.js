...
async function dismissStartupModals(page, frame) {
  ...
  for (const sel of SELECTORS.overlayProbe) {
    ...
    try {
      ...
      throw new NaverError(
        NAVER_EXIT.SELECTOR,
        `오버레이 프로브(${sel}) 실행에 실패해 화면이 가려졌는지 확인할 수 없습니다: ${err.message}\n`
...
async function probeCandidates(frame, candidates, key, visit) {
  ...
  // 후보를 하나도 조회하지 못했다 = "0건"이 아니라 "모른다".
  if (probeErrors.length === candidates.length) {
    throw new NaverError(
      NAVER_EXIT.SELECTOR,
      `${key} 조회에 모두 실패해 상태를 알 수 없습니다 (요소가 없다는 뜻이 아닙니다).\n`
...
/**
 * 에디터에 삽입된 이미지 컴포넌트 수.
 * @returns {Promise<number>} 조회에 성공했고 요소가 없으면 0.
