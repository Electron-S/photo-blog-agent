...
const {
  ...
  BR_SENTINEL, flatElementChildren, followingSiblings, parseHtml, textOf, walk,
} = require('./html-parse');
const {
...
// 문장 경계를 만드는 블록 요소.
//
// **화이트리스트가 아니라 분할 지점이다.** 처음에는 "이 태그 안의 텍스트만 본다"로
// 만들었는데, 그러면 목록 밖에 있는 텍스트가 통째로 사라진다 — `<div>`·`<section>`·
// `<strong>`만으로 감싼 문장, 루트 바로 아래 맨 텍스트에서 error도 warn도 나오지
// 않았다 (실측). 같은 텍스트가 `bodyChars`에는 그대로 들어가므로 "글자수에는 본문,
// 작성 시점 규칙에는 본문 아님"이라는 비일관까지 생겼다. 이 규칙은
// "오늘 다녀왔다"가 LIVE로 나가는 것을 막는 유일한 error다.
//
// 그래서 텍스트는 **전부** 모으고 이 태그들의 앞뒤에 경계만 넣는다. 순회가 한 번뿐이라
// 중첩(`<li>` 안의 `<p>`)에서 텍스트가 두 번 세이지도 않고, 바깥 컨테이너가 안쪽
// 블록 경계를 지우지도 않는다.
const BLOCK_BOUNDARY_TAGS = new Set([
  'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6',
  'li', 'ul', 'ol', 'dl', 'dt', 'dd',
  'div', 'section', 'article', 'aside', 'header', 'footer', 'main', 'nav',
  'blockquote', 'pre', 'address', 'form', 'hr',
  'figure', 'table', 'thead', 'tbody', 'tr', 'td', 'th',
]);

// 본문 텍스트를 블록 경계로 쪼갠 조각들. 경계 표시는 BR_SENTINEL을 재사용한다 —
// `<br>`도 경계이고(korean-text의 splitSentences와 같은 취급), 그래야
// SENTENCE_END_RE의 NUL 분기가 실제로 살아난다.
//
// `<br>`을 경계로 두면 문장 중간 줄바꿈(`오늘 남산에<br>다녀왔습니다.`)에서 error가
// 풀린다. 의도된 트레이드오프다: error는 발행을 차단하고 우회 플래그가 없어 오탐이
// 가장 비싸고, 이 미탐은 같은 판정을 면제 없이 하는 warn(writing-date-word)이 덮는다.
function collectBodySegments(root) {
  const skip = new Set(NON_BODY_TAGS);
  let out = '';
  const rec = (n) => {
    if (n.type === 'text') { out += n.value; return; }
    if (n.type !== 'element') return;
    if (n !== root && skip.has(n.tag)) return;
    if (n.tag === 'br') { out += BR_SENTINEL; return; }
    const boundary = BLOCK_BOUNDARY_TAGS.has(n.tag);
    if (boundary) out += BR_SENTINEL;
    for (const c of n.children) rec(c);
    if (boundary) out += BR_SENTINEL;
  };
  rec(root);
  return out.split(BR_SENTINEL).map((seg) => normalizeWhitespace(seg)).filter(Boolean);
}
...
function buildContext(html, options) {
  ...
  // 어디에도 경계가 없다. 그래서 시제 지시어와 방문 동사가 **서로 다른 블록**에
  // 있어도 창 안에 들어와 error가 났다 (실측: `<h3>이번 주 추천 코스</h3>` +
  // `<p>남산에 다녀왔습니다.</p>` → 차단). h3는 마침표로 끝나지 않고,
  // prompts/blog-draft.md는 1,500자 이상이면 h3 3개를 **요구**하며 그 다음 블록은
  // 항상 <p>다 — 프롬프트가 지시한 모양에서 바로 나오는 오탐이고, 게다가 메시지가
  // 인용하는 "표현"은 초안 어디에도 없는 접합 문자열이라 검색해도 찾을 수 없다.
  //
  // 블록 안에서는 `<br>`도 경계로 친다. korean-text의 splitSentences가 이미 그렇게
  // 하는데 이 규칙만 아니었다 (BR_SENTINEL이 normalizeWhitespace에서 공백으로
  // 접혀, SENTENCE_END_RE의 `\u0000` 분기가 죽어 있었다).
  const bodySegments = collectBodySegments(root);
...
const RULES = [
  ...
  },
  {
    ...
    severity: 'error',
    check: (ctx) => {
      const found = ctx.bodySegments
        .map((seg) => findWritingDateExpression(seg))
        .find(Boolean);
      return found ? [{
  ...
  },
  {
    ...
    severity: 'warn',
    // warn도 같은 세그먼트를 쓴다 — 판정 범위는 넓게 두되, 인용하는 조각이
    // 초안에 실재하지 않는 접합 문자열이면 작성자가 찾을 수 없다.
    check: (ctx) => ctx.bodySegments.flatMap((seg) => findWritingTimeWords(seg)).map((snippet) => ({
  ...
  },
  {
    ...
    check: (ctx) => {
      ...
      return ctx.images.flatMap((n) => {
        ...
        // 속성이 없거나 숫자가 아니면 **img-width-height가 이미 잡는다.** 여기서
        // 또 보고하면 한 원인에 error가 둘이고, 그중 하나는
        // "본문 NaNxNaN, 실제 1024x768"이라 아무 정보도 주지 않는다 (실측).
