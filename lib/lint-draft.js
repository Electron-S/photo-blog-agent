...
// `\d+월`·`\d+일`을 단독으로 두면 **진짜 위반을 면제한다** (실측 5건):
//   "오늘 3일 만에 다시 다녀왔습니다" · "오늘 5월 들어 처음 다녀왔습니다"
//   "오늘 2일차 일정으로 갔습니다"    · "오늘 4일 연속 갔습니다"
// `오늘 + 다녀왔다`는 이 규칙이 존재하는 이유 그 자체다. 숫자+월/일은 **과거를
// 지시하는 조사와 함께일 때만** 과거 표지다 (`5월에`, `5월 10일에`).
// `지난|작년|재작년|예전|그때|그날|저번|며칠 전`은 단독으로도 과거를 뜻한다.
const PAST_MARKER_RE = new RegExp([
  '지난', '작년', '재작년', '예전', '그때', '그날', '저번', '며칠\\s*전',
  '\\d+월\\s*\\d+일\\s*에', '\\d+월\\s*에', '\\d+일\\s*에',
].join('|'));
  ...
  for (const word of WRITING_TIME_WORDS) {
    ...
    while (at !== -1) {
      ...
      while (v) {
        ...
        //
        // **1음절 `이`·`중`을 그대로 두면 안 된다.** `\s*`가 공백까지 먹으므로
        // `방문 이용/이력/이후/이유/이벤트`, `방문 중간/중요/중단`이 전부
        // "서술어가 붙었다"로 판정돼 error 차단됐다 (실측 8건). 이 규칙이 고치겠다고
        // 선언한 FP 클래스 그 자체이고, CLAUDE.md가 요구하는 "이용 안내를 본문에
        // 녹여라"와 정면으로 충돌한다. → **활용 음절을 함께 요구한다.**
        const isNounCompound = v[0] === '방문'
          && !/^\s*(?:했|하|해|였|중(?:이|입|에|일)|이라|이었|이에|이야|입니|이다)/
            .test(window.slice(v.index + v[0].length));
...
// 창을 넓혀도 강화 문맥이 면제되지 않는다 ("수익형 블로그와는 무관합니다"의
// `블로그와는 `처럼 조사구가 끼는 경우까지 닿아야 한다).
const NEGATED_AFTER_RE = /^[^.!?\u0000]{0,10}?(?:아니|아냐|아님|무관|말고)/;
