...
// 것으로 보고 넘긴다:
//   (1) 절 연결어미(-서/-고/-면/-는데/-지만/-니까/-러/-려고)가 사이에 있다
//   (2) 동사 바로 앞에 부정어(못/안)가 있다 — 방문했다는 주장이 아니다
const CLAUSE_BREAK_RE = /(?:서|고|면|는데|지만|니까|러|려고)\s/;
...
function findWritingDateExpression(text) {
  for (const word of WRITING_TIME_WORDS) {
    ...
    for (const m of text.matchAll(re)) {
      const between = m[1];
      if (CLAUSE_BREAK_RE.test(between)) continue;
      if (NEGATION_RE.test(between)) continue;
      return m[0];
    }
  }
  return null;
