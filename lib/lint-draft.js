...
// 지시어가 시간 표현이 아닌 **고정 어구**의 일부인 경우. warn의 신호를 유지하기
// 위한 최소 목록이며, 빠뜨려도 결과는 "warn 한 줄"이지 차단이 아니다.
...
function findWritingTimeWords(text) {
  // 고정 어구를 **먼저 지운다.** 매치 지점에서 앞으로만 비교하면 겹치는 어구를
  // 놓친다 — `어제오늘`의 `오늘`은 그 자리에서 보면 고정 어구로 보이지 않는다.
  const stripped = WRITING_TIME_LEXICALIZED
    .reduce((acc, lex) => acc.split(lex).join(' '), text);

  const found = [];
  for (const word of WRITING_TIME_WORDS) {
    const at = stripped.indexOf(word);
    if (at !== -1) found.push(stripped.slice(at, at + Math.min(24, stripped.length - at)));
  }
  return found;
}
