  ...
  for (const word of WRITING_TIME_WORDS) {
    ...
    while (at !== -1) {
      ...
      while (v) {
        ...
        // **방문 동사는 단어 시작이어야 한다.** `갔`은 합성 이동동사의 뒷음절로도
        // 나타난다 — 내려**갔**/지나**갔**/넘어**갔**/돌아**갔**. 사정거리를 넓히면
        // 그것들이 창에 들어와 정당한 문장을 error로 막는다 (실측 오탐:
        // "오늘 기준 가격이 조금 내려갔어요", "어제 밤에 기온이 영하로 내려갔다고").
        // stem 목록에 예외를 열거하는 대신 **경계**를 요구한다.
        const boundaryOk = v.index === 0 || /\s/.test(window[v.index - 1]);
