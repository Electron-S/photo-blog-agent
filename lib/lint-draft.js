...
// 동사에 걸려 정당한 음식 묘사를 차단한다 (이 규칙이 원래 피하려던 오탐이다).
// "갔다 왔"처럼 방문이 확실한 조합만 넣는다.
const VISIT_VERB_STEMS = [
  '다녀', '방문', '갔', '가봤', '가 봤', '들렀', '들러', '찾았', '찾아갔', '갔다 왔',
];
// 시제 지시어와 방문 동사 사이에 허용할 최대 거리 (한국어 어순상 부사·조사 몇 개).
const WRITING_DATE_GAP = 12;

const WRITING_DATE_RES = WRITING_TIME_WORDS.map((word) => new RegExp(
  `${word}[^.!?\u0000]{0,${WRITING_DATE_GAP}}?(?:${VISIT_VERB_STEMS.join('|')})`,
));
