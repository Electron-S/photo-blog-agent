...
const RULES = [
  ...
  },
  {
    ...
    check: (ctx) => ctx.images.flatMap((n) => {
      ...
      const bad = ['width', 'height'].filter((a) => !/^[1-9]\d*$/.test(String(getAttr(n, a) || '')));
      return bad.length
  ...
  },
  {
    ...
    check: (ctx) => {
      ...
      return [{
        ...
        detail: '다른 슬러그의 결과 파일이거나, 본문 이미지 URL이 업로드 결과와 다릅니다.',
      }];
    },
  },
];
...
function matchedImageCount(ctx) {
  ...
  return ctx.images.filter((n) => ctx.uploadIndex.lookup(getAttr(n, 'src'))).length;
}
