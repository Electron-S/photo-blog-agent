...
// 텍스트가 담기는 블록 요소. 이 경계를 넘어 한 문장으로 읽으면 안 된다.
// figcaption은 NON_BODY_TAGS라 textOf 단계에서 이미 빠진다.
...
function buildContext(html, options) {
  ...
  const bodySegments = [];
  walk(root, (n) => {
    if (n.type !== 'element' || !BLOCK_TEXT_TAGS.has(n.tag)) return;
    for (const seg of textOf(n, NON_BODY_TAGS).split(BR_SENTINEL)) {
      const t = normalizeWhitespace(seg);
      if (t) bodySegments.push(t);
    }
  });
