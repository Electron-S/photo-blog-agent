...
const PAST_MARKER_RE = /(?:지난|작년|재작년|예전|그때|그날|저번|며칠\s*전|\d+월|\d+일)/;
    ...
    while (at !== -1) {
      ...
      while (v) {
        ...
        const isNounCompound = v[0] === '방문'
          && !/^\s*(?:했|하|해|중|이|였)/.test(window.slice(v.index + v[0].length));
...
const NEGATED_AFTER_RE = /^[^.!?\u0000]{0,14}?(?:아니|없|않|말고|무관|아냐|아님)/;
