...
// playwright는 optionalDependencies다 (네이버 발행은 선택 기능이고, 브라우저
// 바이너리가 ~150MB라 Blogger 전용 사용자와 CI에 강제할 수 없다).
// 최상위 require면 미설치 시 스택트레이스로 죽으므로 사용 시점에 지연 로드하고,
// 무엇을 설치해야 하는지 알려주는 exit 10으로 분류한다.
function requirePlaywright() {
  try {
    return require('playwright');
  } catch (err) {
    const wrapped = new Error(
      '네이버 발행에는 playwright가 필요합니다 (optionalDependencies).\n'
      + '  + 설치: npm install --include=optional\n'
