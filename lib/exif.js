...
function normalizeGps(gpsData, label = '', warn = () => {}) {
  ...
  const refOf = (raw) => {
    ...
    if (Buffer.isBuffer(raw)) return raw.toString('latin1').replace(/\0/g, '').trim().toUpperCase();
    if (Array.isArray(raw) && raw.length && typeof raw[0] === 'string') return raw[0].trim().toUpperCase();
    return null;
  };
...
function summarizeDates(photos) {
  const dateDays = (photos || [])
    .filter((p) => p?.date_status !== 'implausible')
