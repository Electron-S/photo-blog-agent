...
function splitSentences(plain) {
  const decoded = decodeEntities(plain == null ? '' : plain);
  if (decoded.includes('\n')) {
    return decoded.split(/\n+/).flatMap((seg) => splitOneLine(seg));
  }
  return splitOneLine(decoded);
}
...
// findEmoji에서 제외할 맨 화살표 (VS16이 붙으면 grapheme 길이가 2라 제외되지 않는다).
const ARROW_NOT_EMOJI_RE = new RegExp(`^[${TYPOGRAPHIC_ARROW}]$`, 'u');
