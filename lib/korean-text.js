...
// html-parse의 textOf가 <br>을 '\n'으로 내보낸다. 줄바꿈은 실제 문장 경계이므로
// 먼저 조각으로 나눈 뒤 각 조각을 문장 분리한다. (예전에는 normalizeWhitespace가
// '\n'을 공백으로 접어버려 "첫 줄<br>둘째 줄"이 1문장으로 세어졌다.)
...
// 재현 불가능한 규칙이 된다.
//
// 판정 기준은 **VS16(U+FE0F) 유무**다:
//   ↔  (맨 화살표)      → 타이포그래피 문자. decorative-arrow(warn)만 담당.
//   ↔️ (화살표+VS16)    → 작성자가 이모지 표현을 의도한 것. no-emoji(error)만 담당.
//
// U+27A1(➡) U+2B05~07(⬅⬆⬇) U+2794(➔)는 화살표 모양이지만 타이포그래피 블록이
// 아니라 RGI 이모지 기저 문자다. style-guide가 금지하는 "장식용"에 가장 가까우므로
// 맨 형태도 이모지(error)로 둔다 — 아래 두 정규식 어디에도 넣지 않는다.
const TYPOGRAPHIC_ARROW = '\\u2190-\\u21FF';

// VS16이 뒤따르지 않는 맨 화살표만 장식용 화살표로 본다.
const DECORATIVE_ARROW_RE = new RegExp(`[${TYPOGRAPHIC_ARROW}](?!\\uFE0F)`, 'gu');

// findEmoji에서 제외할 화살표: 맨 형태와 VS15(U+FE0E) 형태.
// VS15는 작성자가 **명시적으로 "이모지 아님"을 요청한** 텍스트 표현 선택자라
// 맨 화살표보다 엄하게 다룰 이유가 없다. VS16(U+FE0F)은 반대로 이모지 표현을
// 의도한 것이므로 여기서 제외하지 않는다 (no-emoji error가 담당).
const ARROW_NOT_EMOJI_RE = new RegExp(`^[${TYPOGRAPHIC_ARROW}]\\uFE0E?$`, 'u');
...
function findEmoji(text) {
  ...
  for (const g of graphemesOf(decoded)) {
    ...
    // VS16이 붙은 형태는 grapheme 길이가 2라 여기서 제외되지 않는다 — 의도된 동작이다.
    if (ARROW_NOT_EMOJI_RE.test(g)) continue;
    hits.add(g);
  }
  return [...hits];
}
