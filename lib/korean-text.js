...
function maskNonBoundaries(text) {
  ...
  // 번호 목록 접두사(`1. `, `2) `)는 문장 끝이 아니다. 마스킹하지 않으면
  // 번호만 붙여도 "이미지 다음 2문장" 요건이 충족돼 규칙이 우회된다
  // (위 URL 오탐과 같은 마스킹 결함의 반대쪽이다).
  s = s.replace(/(?<=^|[\s\u0000])\d{1,2}\.(?=\s)/g, (m) => SENTENCE_MASK.repeat(m.length));
