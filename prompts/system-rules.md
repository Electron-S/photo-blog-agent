...
- `<figure>` 요소에 `style="margin:1.5em 0;text-align:center;position:relative;"` 을 적용하여 이미지와 주변 텍스트 사이 여백을 확보하고, 이미지와 캡션을 가운데 정렬한다. `position:relative`는 이미지 보호 오버레이를 위해 필요하다.
